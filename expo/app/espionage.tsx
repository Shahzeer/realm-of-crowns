...
import { X, Eye, Clock, Target } from "lucide-react-native";
...
export default function EspionageScreen() {
  ...
  const { state, startSpyMission } = useGame();
  ...
  const handleMission = useCallback((missionId: string) => {
    ...
    if (mission.targetType === 'kingdom' && !targetId) {
      Alert.alert("Select Target", "Choose a target kingdom first.");
      return;
    }
    if (mission.targetType === 'province' && !targetId) {
      Alert.alert("Select Target", "Choose a target province first.");
      return;
    }

    if (Platform.OS !== "web") { void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy); }
    startSpyMission(missionId, targetId || '');
  }, [state.activeSpyMission, state.resources.gold, selectedTarget, startSpyMission]);

  const targetKingdoms = state.kingdoms.filter(k => k.attitude !== 'allied');
  const _enemyProvinces = state.provinces.filter(p => p.owner !== 'player');
                ...
                <View style={[s.missionCard, isActive && s.missionCardDisabled]}>
                  ...
                  <View style={s.missionStats}>
                    ...
                    <View style={s.missionStat}>
                      ...
                      <Text style={s.missionStatValue}>{adjustedTurns}t</Text>
                    </View>
                    <View style={s.missionStat}>
                      <Text style={s.missionStatLabel}>Success</Text>
                      <Text style={[s.missionStatValue, {
                        color: adjustedSuccess > 60 ? Colors.status.success : adjustedSuccess > 40 ? Colors.status.warning : Colors.crimson.bright
                      }]}>{Math.round(adjustedSuccess)}%</Text>
                    </View>
                  </View>
